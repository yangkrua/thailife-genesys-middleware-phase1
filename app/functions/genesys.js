 ...
 */
let AnalyticsConversationsDetailsQuery_ANumberReport = () => {
  client
 ...
 */
let ManualGenA_Number_Report_ByDate = (pDate) => {
  client
    .loginClientCredentialsGrant(CLIENT_ID, CLIENT_SECRET)
    .then(async () => {
      log.info("AnalyticsConversationsDetailsQuery->login(), success fully!");
 ...
 */
let ManualGenLoginLogoutByDateReport = (pDate) => {
  client
    .loginClientCredentialsGrant(CLIENT_ID, CLIENT_SECRET)
    .then(async () => {
      log.info("GetUsers->login(), success fully!");
 ...
 */
let ManualGenAbandonWith_tFlow_ByDate = (pDate) => {
  client
    .loginClientCredentialsGrant(CLIENT_ID, CLIENT_SECRET)
    .then(async () => {
      log.info("AnalyticsConversationsDetailsQuery->login(), success fully!");
...
let isTransactionInboundCaptureByPhoneListsANI = async (data_participants) => {
  for (const item of data_participants) {
    for (const session of item.sessions) {
      if ((await session.ani) !== undefined) {
        for (const phone of config.GENESES.filter_by_ani) {
          if ((await session.ani) === phone) {
            return true;
          }
        }
      }
